...
public class Updater : ModuleUpdater {
    ...
    private PermissionPolicyRole CreateDepartmentManagerRole() {
        ...
        //MyTask
        role.AddObjectPermissionFromLambda<MyTask>(SecurityOperations.FullObjectAccess, t => t.AssignedTo.Department.ApplicationUsers.Any(au2 => au2.ID == (Guid)CurrentUserIdOperator.CurrentUserId()), SecurityPermissionState.Allow);
        ...
        //MyTask
        role.AddObjectPermissionFromLambda<MyTask>(SecurityOperations.Read, t => t.AssignedTo.ID == (Guid)CurrentUserIdOperator.CurrentUserId(), SecurityPermissionState.Allow);
        role.AddObjectPermissionFromLambda<MyTask>(SecurityOperations.Read, t => t.AssignedTo.Department.ApplicationUsers.Any(au => au.ID == (Guid)CurrentUserIdOperator.CurrentUserId()) && t.IsSharedTask, SecurityPermissionState.Allow);
